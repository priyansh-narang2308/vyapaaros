...
Both protocols utilize the **VyapaarOS NeMo Agent Toolkit** to perform real-time business logic optimization. The system uses an "Async Parallel Orchestration" pattern to ensure fast, responsive agent reasoning while sharing the same intelligent agent layer and backend services.
...
* **Configurable NIM Endpoint**: Supports both VyapaarOS hosted API and local Docker deployment via environment variable.
