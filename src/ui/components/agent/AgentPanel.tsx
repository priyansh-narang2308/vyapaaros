...
export function AgentPanel({ protocol }: AgentPanelProps) {
  ...
  const handleContinueToPayment = useCallback(() => {
    // If in Native mode and we have a Razorpay order, launch Razorpay JS
    if (
      activeMode === "native" &&
      context.session?.provider_session_id &&
      typeof window !== "undefined" &&
      window.Razorpay
    ) {
      const razorpayOrderId = context.session.provider_session_id;

      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_VyapaarMockKey",
        amount: context.session.totals.find((t) => t.type === "total")?.amount ?? 0,
        currency: context.session.currency.toUpperCase(),
        name: "VyapaarOS",
  ...
  }, [proceedToPayment, activeMode, context.session, submitRazorpayPayment]);
